...
On invite acceptance, a Supabase Auth trigger automatically creates a `profiles` record with the role assigned at invite time.
...
All RLS policies are defined in Supabase migrations and managed via the Supabase CLI.
...
The person merge operation (linking a local visitor record to a PC duplicate) is super-admin only and is performed server-side via Hono. It requires:
